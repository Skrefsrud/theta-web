...
export function ProblemSolution() {
  return (
    <section id="science" className="relative py-24 md:py-32 overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="bg-white/5 backdrop-blur-md rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] border border-white/10 p-10 md:p-20 space-y-32">
          {sections.map((section, index) => (
            <div
              key={index}
              className={`flex flex-col ${index % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"} gap-12 items-start`}
            >
              {/* Text Content */}
              <div className="flex-1 space-y-6">
                ...
                <p className="text-lg text-slate-300 leading-relaxed">{section.empathy}</p>
                <div className="pt-4 border-t-2 border-cyan-500/50">
                  <p className="text-xl text-slate-200 leading-relaxed font-medium">{section.solution}</p>
                </div>
              </div>

              {/* Image */}
              <div className="flex-1 relative aspect-[3/4] w-full rounded-2xl overflow-hidden shadow-2xl ring-1 ring-white/10">
                <Image src={section.image || "/placeholder.svg"} alt={section.problem} fill className="object-cover" />
              </div>
            </div>
          ))}
        </div>
